...
def data_prepare(subject, roi_all, img_feature, layer_all, voxel_all):
    ...
    for roi in roi_all:
        ...
        correlation_parameters_plot(data, x_roi, img_feature, 's4', roi)
...
def correlation_parameters_plot(data, x_roi, y_layers_all, sbj, roi):
    ...
    cor_cnn2, cor_cnn2 = x_layer(data, x_roi, y_layers_all, 'cnn2')
    cor_cnn3, cor_cnn3 = x_layer(data, x_roi, y_layers_all, 'cnn3')
    cor_cnn4, cor_cnn4 = x_layer(data, x_roi, y_layers_all, 'cnn4')
    cor_cnn5, cor_cnn5 = x_layer(data, x_roi, y_layers_all, 'cnn5')
    cor_cnn6, cor_cnn6 = x_layer(data, x_roi, y_layers_all, 'cnn6')
    cor_cnn7, cor_cnn7 = x_layer(data, x_roi, y_layers_all, 'cnn7')
    cor_cnn8, cor_cnn8 = x_layer(data, x_roi, y_layers_all, 'cnn8')

    cor_hmax1, cor_hmax1 = x_layer(data, x_roi, y_layers_all, 'hmax1')
    cor_hmax2, cor_hmax2 = x_layer(data, x_roi, y_layers_all, 'hmax2')
    cor_hmax3, cor_hmax3 = x_layer(data, x_roi, y_layers_all, 'hmax3')

    cor_gist, cor_gist = x_layer(data, x_roi, y_layers_all, 'gist')
    cor_sift, cor_sift = x_layer(data, x_roi, y_layers_all, 'sift')
    ...
    plt.bar(range(cor_cnn1.shape[0]), cor_cnn1)

    plt.subplot(5, 3, 2)
    plt.title('%s_%s_cnn2' % (sbj, roi))
    plt.bar(range(cor_cnn2.shape[0]), cor_cnn2)

    plt.subplot(5, 3, 3)
    plt.title('%s_%s_cnn3' % (sbj, roi))
    plt.bar(range(cor_cnn3.shape[0]), cor_cnn3)

    plt.subplot(5, 3, 4)
    plt.title('%s_%s_cnn4' % (sbj, roi))
    plt.bar(range(cor_cnn4.shape[0]), cor_cnn4)

    plt.subplot(5, 3, 5)
    plt.title('%s_%s_cnn5' % (sbj, roi))
    plt.bar(range(cor_cnn5.shape[0]), cor_cnn5)

    plt.subplot(5, 3, 6)
    plt.title('%s_%s_cnn6' % (sbj, roi))
    plt.bar(range(cor_cnn6.shape[0]), cor_cnn6)

    plt.subplot(5, 3, 7)
    plt.title('%s_%s_cnn7' % (sbj, roi))
    plt.bar(range(cor_cnn7.shape[0]), cor_cnn7)

    plt.subplot(5, 3, 8)
    plt.title('%s_%s_cnn8' % (sbj, roi))
    plt.bar(range(cor_cnn8.shape[0]), cor_cnn8)

    plt.subplot(5, 3, 9)
    plt.title('%s_%s_hmax1' % (sbj, roi))
    plt.bar(range(cor_hmax1.shape[0]), cor_hmax1)

    plt.subplot(5, 3, 10)
    plt.title('%s_%s_hmax2' % (sbj, roi))
    plt.bar(range(cor_hmax2.shape[0]), cor_hmax2)

    plt.subplot(5, 3, 11)
    plt.title('%s_%s_hmax3' % (sbj, roi))
    plt.bar(range(cor_hmax3.shape[0]), cor_hmax3)

    plt.subplot(5, 3, 12)
    plt.title('%s_%s_gist' % (sbj, roi))
    plt.bar(range(cor_gist.shape[0]), cor_gist)

    plt.subplot(5, 3, 13)
    plt.title('%s_%s_sift' % (sbj, roi))
    plt.bar(range(cor_sift.shape[0]), cor_sift)

    plt.tight_layout()
    plt.savefig('plots/%s_layers/%s_%s_correlation.png' % (sbj, sbj, roi))
    plt.close('all')
