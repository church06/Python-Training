...
data_path = 'data\\'
result_path = 'HDF5s\\Result.hdf5'
XY_T_STD_path = 'HDF5s\\xy_train&std_data.hdf5'
MResult_path = 'HDF5s\\mergedResult.hdf5'
hdf5_path = 'HDF5s\\'
cat_data_path = 'HDF5s\\cat_data.hdf5'
...
# ========================================================================
def create_xy_std_data(sbj_num: str, roi: str, layer: str):
    print("Creating ['%s'] test data by normalization: [none, z-score, min-max, decimal]" % layer)

    file = h5py.File(XY_T_STD_path, 'a')
    roi_up = roi.upper()
...
def read_xy_std_data(sbj_num: str):
    path = 'HDF5s\\xy_train&std_data.hdf5'
    ...
    l_keys = list(group[r_keys[0]].keys())
...
# ========================================================================
def read_result_data(sbj_num: str):
    ...
    r_list.remove('VC')
    l_list = ['cnn1', 'cnn2']
    n_list = list(sbj[r_list[0]][l_list[0]].keys())
    d_list = list(sbj[r_list[0]][l_list[0]][n_list[0]].keys())
...
def read_merged_data(sbj_num):
    ...
    l_list = list(loc[r_list[0]].keys())
    n_list = list(loc[r_list[0]][l_list[0]].keys())
    d_list = list(loc[r_list[0]][l_list[0]][n_list[0]].keys())
...
def read_cat(sbj_num: str):
    print('Getting categories.')
    file = h5py.File(cat_data_path, 'r')
    target = file[sbj_num]
...
def save_to_result(S: str, R: str, L, N: int, data_dict: dict):
    print('Save to Results.hdf5 !!!!!!')
    hdf5_dir = 'HDF5s\\Result.hdf5'
    # r:        Read only
